...
const showResult = computed(() => {
  ...
  if (dataType === 'video' || dataType === 'shortVideo') {
    ...
    if (channelsHidden.value.some(ch => ch.name === props.data.authorId) || channelsHidden.value.some(ch => ch.name === props.data.author) || (forbiddenTitles.value.some((text) => lowerCaseAuthor.includes(text.toLowerCase())))) {
    ...
    if ((attrsToCheck.some(a => a != null && channelsHidden.value.some(ch => ch.name === a))) ||
      (forbiddenTitles.value.some((text) => lowerCaseName.includes(text.toLowerCase())))) {
    ...
    if ((forbiddenTitles.value.some((text) => lowerCaseTitle.includes(text.toLowerCase()))) ||
      (forbiddenTitles.value.some((text) => lowerCaseChannelName.includes(text.toLowerCase())))) {
      return false
    }
