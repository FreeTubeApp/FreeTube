...
export default Vue.extend({
  ...
  data: function () {
    return {
      ...
      shownElementList: [],
      apiUsed: '',
      videoSelectValues: ['newest', 'oldest', 'popular'],
      playlistSelectValues: ['last', 'newest']
    }
  },
  computed: {
      ...
      switch (this.currentTab) {
        case 'videos':
          if (
            ...
            (this.videoContinuationString !== '' &&
              this.videoContinuationString !== null)
          ) {
            return true
          }
          break
        case 'playlists':
          if (
            ...
            this.playlistContinuationString !== null
          ) {
            return true
          }
          break
        case 'search':
          if (
            ...
            this.searchContinuationString !== null
          ) {
            return true
          }
          break
  ...
  methods: {
      ...
      ytch
        ...
        .then((response) => {
          ...
          if (response.authorBanners !== null) {
            const bannerUrl =
              response.authorBanners[response.authorBanners.length - 1].url
    ...
    getChannelVideosLocal: function () {
      this.isElementListLoading = true
      ytch
        .getChannelVideos(this.id, this.videoSortBy)
        .then((response) => {
          const items = response.items.map((item) => {
            item.verified = this.isVerified
            return item
          })
          this.latestVideos = items
    ...
    channelLocalNextPage: function () {
      ytch
        .getChannelVideosMore(this.videoContinuationString)
        .then((response) => {
    ...
    getChannelInfoInvidious: function () {
      ...
      this.$store
        .dispatch('invidiousGetChannelInfo', this.id)
        .then((response) => {
          ...
          this.thumbnailUrl = response.authorThumbnails[3].url.replace(
            'https://yt3.ggpht.com',
          ...
          this.relatedChannels = response.relatedChannels.map((channel) => {
            channel.authorThumbnails[
              channel.authorThumbnails.length - 1
            ].url = channel.authorThumbnails[
              channel.authorThumbnails.length - 1
            ].url.replace(
              'https://yt3.ggpht.com',
              `${this.invidiousInstance}/ggpht/`
          ...
          if (typeof response.authorBanners !== 'undefined') {
            this.bannerUrl = response.authorBanners[0].url.replace(
              'https://yt3.ggpht.com',
              `${this.invidiousInstance}/ggpht/`
            )
          }
    ...
    channelInvidiousNextPage: function () {
      ...
      this.$store
        .dispatch('invidiousAPICall', payload)
        .then((response) => {
    ...
    getPlaylistsLocal: function () {
      ytch
        .getChannelPlaylistInfo(this.id, this.playlistSortBy)
        .then((response) => {
          console.log(response)
    ...
    getPlaylistsLocalMore: function () {
      ytch
        .getChannelPlaylistsMore(this.playlistContinuationString)
        .then((response) => {
          console.log(response)
    ...
    searchChannelLocal: function () {
      if (this.searchContinuationString === '') {
        ytch
          .searchChannel(this.id, this.lastSearchQuery)
          .then((response) => {
            console.log(response)
        ...
        ytch
          .searchChannelMore(this.searchContinuationString)
          .then((response) => {
            console.log(response)
    ...
    ...mapActions(['showToast', 'updateProfile'])
  }
})
